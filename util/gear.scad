...
//	circles=8,
//    number_of_teeth=24);
//internal_gear(circular_pitch = 700,
//    number_of_teeth=24,
//    gear_thickness=12,
//    outer_radius=100);
...
module internal_gear(circular_pitch = 700,
    number_of_teeth=24,
    gear_thickness=12,
    outer_radius=500,
    clearance=5) 
{
    number_of_teeth = floor((outer_radius-clearance) * 2 * 180 / circular_pitch);
    difference() {
        cylinder(r = outer_radius, h = gear_thickness);
        translate([0,0,-1])
            gear(number_of_teeth = number_of_teeth, circular_pitch = circular_pitch, gear_thickness = gear_thickness + 2, rim_thickness = gear_thickness + 2, hub_thickness = gear_thickness + 2, bore_diameter = 0);
    }
}
